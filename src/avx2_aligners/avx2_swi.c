...
avx2_fill_table_8_to_8_i32 (int* seqs1, int* seqs2, int x, int y,
			    ...
			    int max_score[8], int ipos[8], int jpos[8])
{
...
int*
avx2_fill_table_8_to_8_i32_with_match (int* seqs1, int* seqs2, int x, int y,
				       int match, int mismatch, int gap_open,
				       int gap_extend, int max_score[8],
				       int ipos[8], int jpos[8])
{
  ...
  for (int i = 1; i < x; i++)
	...
	{
	  s2 = _mm256_load_si256 ((__m256i *) (seqs2 + 8 * (j - 1)));
	  temp = _mm256_cmpeq_epi32 (s1, s2);
	  score = _mm256_blendv_epi8 (vmismatch, vmatch, temp);
	  temp = _mm256_cmpeq_epi32 (s1, vzero);
	  score = _mm256_blendv_epi8 (score, vmismatch, temp);
	  temp = _mm256_cmpeq_epi32 (s2, vzero);
	  score = _mm256_blendv_epi8 (score, vmismatch, temp);
	  //index = _mm256_add_epi32 (temp_index, s2);
	  //score = _mm256_i32gather_epi32(subs_matrix, index, 4);
			...
			int gap_extend, int dup_strings)
{
  ...
  int* flags = avx2_fill_table_8_to_8_i32_with_match (packed_seqs1,
						      packed_seqs2, max_i + 1,
						      max_j + 1, match,
						      mismatch, gap_open,
